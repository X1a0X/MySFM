...
def extract_features(image_names):
    ...
    for image_name in image_names:
        ...
        img=cv2.drawKeypoints(cv2.cvtColor(image, cv2.COLOR_BGR2GRAY),key_points,cv2.cvtColor(image, cv2.COLOR_BGR2GRAY),color=(255,0,0))  #color为标记特征点的颜色，按（B，G，R）排的
...
def fig(structure, colors):
    ...
    ax.view_init(elev = 1350, azim = 900)
    plt.show()
